  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Espaces clients</h1>
      ...
      {isAdmin && (
        <div style={{ backgroundColor: G5, borderRadius: 12, padding: 20, marginBottom: 24, border: `1px solid ${G3}` }}>
          ...
          <form onSubmit={createWorkspace} style={{ display: 'flex', gap: 10 }}>
            ...
            />
            <button type="submit" disabled={creating || !newName.trim()} style={{ backgroundColor: ORANGE, border: 'none', borderRadius: 8, padding: '10px 20px', color: 'white', fontSize: 14, fontWeight: 700, cursor: creating ? 'not-allowed' : 'pointer', opacity: creating ? 0.7 : 1 }}>
      ...
      {workspaces.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {workspaces.map(ws => (
            <div key={ws.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: G5, borderRadius: 10, padding: '16px 20px', border: `1px solid ${G3}` }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 15 }}>{ws.name}</div>
                <div style={{ color: '#7a9e8e', fontSize: 12, marginTop: 3 }}>{new Date(ws.createdAt).toLocaleDateString('fr-FR')}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ backgroundColor: ws.role === 'owner' ? ORANGE + '22' : G3, color: ws.role === 'owner' ? ORANGE : '#7a9e8e', borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                  {ROLE_LABEL[ws.role] ?? ws.role}
                </span>
                {(isAdmin || ws.role === 'owner') && (
                  <Link href={`/admin/workspaces/${ws.id}`} style={{ backgroundColor: G4, color: CREAM, borderRadius: 6, padding: '6px 14px', fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
                    Gérer
                  </Link>
                )}
                {isAdmin && (
                  <button onClick={() => deleteWorkspace(ws.id, ws.name)} style={{ backgroundColor: 'transparent', border: '1px solid #e05050', borderRadius: 6, padding: '5px 12px', color: '#e05050', fontSize: 12, cursor: 'pointer' }}>
                    Supprimer
                  </button>
                )}
              </div>
